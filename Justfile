...
gh-release:
    ...
    git tag -a "v{{new_version}}" -m "auto generated by the justfile for milky v{{new_version}}"
    just cross
    mkdir -p ./target/"release-notes-{{new_version}}"
    git cliff -t "v{{new_version}}" --current > ./target/"release-notes-{{new_version}}/RELEASE.md"
    just checksum >> ./target/"release-notes-{{new_version}}/RELEASE.md"
    git push origin "v{{new_version}}"
    gh release create "v{{new_version}}" --target "$(git rev-parse HEAD)" --title "milky v{{new_version}}" -d -F ./target/"release-notes-{{new_version}}/RELEASE.md" ./target/"bin-{{new_version}}"/*
...
checksum:
    ...
    @echo "## sha256sum"
    @echo '```'
    @sha256sum ./target/"bin-{{new_version}}"/*
    @echo '```'
    @echo "## md5sum"
    @echo '```'
    @md5sum ./target/"bin-{{new_version}}"/*
    @echo '```'
    @echo "## blake3sum"
    @echo '```'
    @b3sum ./target/"bin-{{new_version}}"/*
    @echo '```'

[group('binaries')]
tar BINARY TARGET:
    tar czvf ./target/"bin-{{new_version}}"/{{BINARY}}_{{TARGET}}.tar.gz -C ./target/{{TARGET}}/release/ ./{{BINARY}}

[group('binaries')]
zip BINARY TARGET:
    zip -j ./target/"bin-{{new_version}}"/{{BINARY}}_{{TARGET}}.zip ./target/{{TARGET}}/release/{{BINARY}}

[group('binaries')]
tar_static BINARY TARGET:
    tar czvf ./target/"bin-{{new_version}}"/{{BINARY}}_{{TARGET}}_static.tar.gz -C ./target/{{TARGET}}/release/ ./{{BINARY}}

[group('binaries')]
zip_static BINARY TARGET:
    zip -j ./target/"bin-{{new_version}}"/{{BINARY}}_{{TARGET}}_static.zip ./target/{{TARGET}}/release/{{BINARY}}
...
cross:
    ...
    mkdir -p ./target/"bin-{{new_version}}"
